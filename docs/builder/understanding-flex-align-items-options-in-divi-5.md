---
title: "Understanding Flex Align Items Options in Divi 5"
category: builder
tags: [builder]
related: []
divi_version: "5.x"
last_updated: 2026-04-06
source_url: "https://help.elegantthemes.com/en/articles/11882896-understanding-flex-align-items-options-in-divi-5"
---

# Understanding Flex Align Items Options in Divi 5

<!-- AUTO-CREATED: 2026-04-06 — stub from ET Help Center, needs enrichment -->

## Overview

For detailed information, see the [official Elegant Themes documentation](https://help.elegantthemes.com/en/articles/11882896-understanding-flex-align-items-options-in-divi-5).

<!-- TODO: Write a 2-3 paragraph overview of this feature/module -->

## Settings & Options

### General

| Setting | Type | Description |
|---------|------|-------------|
| top |  | gn multiple modules to thetop,center,bottom, orstretchthem to fill the height of their parent container. |

### Design Tab

| Setting | Type | Description |
|---------|------|-------------|
| Design Tab → Layout → Horizontal Gap |  | sign Tab → Layout → Horizontal Gapenter0(zero). This will remove the space between the two columns. |
| Design Tab → Layout  →Align Items |  | → Layout  →Align Items, chooseCenter |

## Code Examples

<!-- TODO: Add CSS/PHP code examples -->

## Common Patterns

<!-- TODO: Document 2-3 common usage patterns -->

## Troubleshooting

<!-- TODO: Document common issues and solutions -->

## AI Interaction Notes

| Task | Confidence | Notes |
|------|-----------|-------|
| Basic placement | 🔬 Needs Testing | Untested — stub page |
| Settings configuration | 🔬 Needs Testing | Untested — stub page |
| Custom styling | 🔬 Needs Testing | Untested — stub page |

## Related

<!-- TODO: Add links to related documentation pages -->
